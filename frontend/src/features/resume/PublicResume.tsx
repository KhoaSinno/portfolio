...
import { defaultResume, parseStoredResume, RESUME_PUBLISHED_KEY, type ResumeData } from "./resume-schema";

export function PublicResume() {
  const [resume, setResume] = useState<ResumeData>(defaultResume);
  useEffect(() => { setResume(parseStoredResume(window.localStorage.getItem(RESUME_PUBLISHED_KEY)) ?? defaultResume); }, []);
